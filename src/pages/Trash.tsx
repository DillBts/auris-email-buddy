      ...
      <div className="flex-1 overflow-y-auto">
        {trashedEmails.length === 0 ? (
          ...
          trashedEmails.map((email, i) => (
            <motion.div key={email.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              ...
              <div className="w-10 h-10 rounded-full bg-muted flex items-center justify-center text-sm font-bold text-muted-foreground shrink-0">{email.from[0]}</div>
